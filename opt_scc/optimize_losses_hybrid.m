function [opt_imp] = optimize_losses_hybrid( implementation,Vin,...
Io,Io_ac,mode)
...
elseif mode == 2
    %Optimitzation for constrained capacitor area
    x0 = log([Fsw_0 Asw_0])';
...
elseif mode==4 
    ...
    %Define objective function
    fobjective = @(x)opt_losses_Lout_Ac(implementation,...
                                 exp(x(1)),exp(x(2)),Vin,Io,Io_ac)
    
    x0 = log([L_0 Ac_0])';
                  
                             
    [x, fval] = fminunc(fobjective,x0);
    
    %      [x, fval, falg] = fmincon(@opt_FoM_func,exp([10; -10; -10]),[],[],[],[],...
    %          exp(lb),exp(ub),[],options,implementation,Vin,Io,rel_ripple_Io);
    
    
    [~, Fsw, Asw] =fobjective(x);
    ...
    [x, fval, falg] = fmincon(@opt_FoM_func,x0,[],[],[],[],lb,ub,[],...
        options,implementation,Vin,Io,Io_ac,1/1e-6);
